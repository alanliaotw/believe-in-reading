    ...
    <>
      ...
      <main className="relative min-h-screen bg-black text-white font-sans selection:bg-emerald-500/30">
        ...
        <nav className="relative z-50 p-6 md:p-10 max-w-7xl mx-auto flex justify-between items-center">
          <div className="relative h-10 w-40"><Image src="/brand-logo.png" alt="聚焦誌" fill className="object-contain" priority /></div>
            ...
            <div className="animate-fade-in">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                {filteredData.length > 0 ? filteredData.map((item: any, i) => {
                  ...
                  const imgValue = item.imageUrl || item["封面圖片連結 (imageUrl)"] || "";
                  ...
                  return (
                    <div key={i} className="group bg-white/5 rounded-[2rem] border border-white/10 overflow-hidden hover:border-emerald-500/50 transition-all shadow-xl">
                      ...
                      <div className="p-8">
                        ...
                        <a href={videoLink} target="_blank" rel="noopener noreferrer" className="text-emerald-400 font-bold hover:underline text-xs tracking-widest uppercase">立即觀看 →</a>
                      </div>
                    </div>
                  );
                }) : <div className="col-span-full text-center py-20 text-gray-500 bg-white/5 rounded-3xl border border-white/10">尚未上傳 3/27 活動成果，敬請期待。</div>}
              </div>
            </div>
          )}
        </div>
