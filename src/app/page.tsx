    ...
    <main className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-black text-white px-6">
      ...
      <nav className="absolute top-0 left-0 z-20 w-full p-6 md:p-10 flex justify-between items-center pointer-events-none">
        <div className="flex items-center pointer-events-auto">
          <Image src="/brand-logo.png" alt="相信閱讀 Logo" width={180} height={60} className="h-10 md:h-12 w-auto object-contain" priority />
        </div>
      </nav>
      ...
      <div className="absolute top-24 md:top-32 left-0 z-[40] w-full overflow-hidden">
        <div className="flex overflow-x-auto px-6 md:px-10 py-2 gap-3 md:gap-4 no-scrollbar">
          {categories.map((cat) => (
            <button key={cat} className="flex-none whitespace-nowrap rounded-full border border-white/20 bg-black/40 backdrop-blur-md px-5 py-2 text-xs md:text-sm font-medium text-gray-300 transition-all hover:bg-emerald-500 hover:text-white active:scale-95">
              {cat}
            </button>
          ))}
        </div>
      ...
      <div className="relative z-10 w-full max-w-4xl text-center animate-in fade-in zoom-in duration-1000 mt-28 md:mt-0">
        ...
        <div className="flex justify-center items-center">
          <a href="https://www.youtube.com/channel/UCiSSOVa6qHyigTSMpI3uQJQ" target="_blank" rel="noopener noreferrer" className="w-full sm:w-auto rounded-full bg-white px-12 py-4 text-black font-bold transition-all active:scale-95 hover:bg-emerald-500 hover:text-white text-center shadow-xl text-sm md:text-base">
            觀看潮永續影片
          </a>
        </div>
      </div>
