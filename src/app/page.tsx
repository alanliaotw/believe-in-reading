            ...
            {activeCategory === "關於我們" ? (
              <div className="max-w-4xl mx-auto py-10">
                {filteredData.map((item: any, i) => {
                  // 🛠️ 讀取權限徹底校正
                  const content = item.description || item.摘要 || ""; // C 欄
                  const alignClass = item.videoUrl || item.影片連結 || "text-center"; // D 欄
                  const sizeClass = item.imageUrl || item.封面圖片連結 || "text-xl"; // E 欄
                  
                  // F 欄：相容多種可能的標題名稱（如果後台標題是「字體顏色」或「樣式」等）
                  const styleClass = item.style || item.columnF || item.其他備註 || item.f_column || ""; 
