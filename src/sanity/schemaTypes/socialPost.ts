...
import { SOCIAL_PLATFORMS, SOCIAL_POST_STATUSES } from '@/lib/social'
...
export const socialPostType = defineType({
    ...
    defineField({
      ...
      validation: (Rule) =>
        Rule.required()
          ...
          .max(10)
          .custom((urls) =>
            Array.isArray(urls) && urls.length > 0 ? true : '至少要放一張圖片連結'
          ),
      group: 'content',
    }),
    defineField({
  ...
  preview: {
    ...
    prepare({ title, subtitle, scheduledAt, images }) {
      ...
      return {
        title,
        subtitle: `${STATUS_TITLES[(subtitle as keyof typeof STATUS_TITLES) || 'scheduled']}｜${scheduleLabel}${imageLabel}`,
      }
    },
  },
  orderings: [
